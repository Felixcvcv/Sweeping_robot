...
GitHub Pages 后在线访问。机器人在一个房间里清扫随机分布的灰尘，房间里的
沙发 / 床 / 茶几 / 柜子 / 书桌都做了俯视建模（坐垫、枕头、玻璃面、柜门、投影等细节），
机器人外观按结构图绘制（**四轮驱动**、中部电池、尾部风扇舱 + 旋转刷盘）。
...
| 自动模式状态机：前进 → 障碍 <20cm → 快速后退 3s → 左转 5s（`app_control.c`） | 完全相同的状态名（`manual / auto-forward / auto-backup / auto-turn`）与时序；阈值按真机取 20cm（固件默认 25cm） |
...
| `W` / `↑` | `0x02` | 前进 |
| `S` / `↓` | `0x03` | 后退 |
| `A` / `←` | `0x04` | 左转（差速 40/100，松开回正） |
| `D` / `→` | `0x05` | 右转（差速 100/40，松开回正） |
...
仿真附加设定（为便于演示，与固件略有差异）：

1. 机器人一旦开始行驶（手动或自动）尾部风扇即联动开启，停止时关闭
   （固件里吸尘由 `0x09` 单独控制）；
2. 前进 / 后退指令会把双轮速度重置为当前档位等速，且**松开 `←` / `→` 转向键自动
   回正直行**——修复了“按一次转向就只能一直转弯、无法恢复直行”的问题；
3. 避障阈值按真机行为取 **20cm**（固件 `app_config.h` 中默认 25cm）。
